...
void flow_test(){
    ...
    struct capture_t *original = malloc(sizeof(struct capture_t));
    load_capture(original, "dump.pcap");

    printf("%d", packet_count(original));
    printf("%d", data_transfered(original));

    destroy_capture(original);
